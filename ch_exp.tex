...
包括我们提出的 DIN 在内的所有深度模型都没有遇到严重过拟合问题。
...
此外，我们按预测值为表示候选广告的点着色。图 \ref{fig:TDdiagram} 也可看作这位母亲在 embedding 空间中对潜在候选项的兴趣密度分布热力图。它表明 DIN 能够在候选项的 embedding 空间中为某个用户形成多峰兴趣密度分布，从而捕获其多样化兴趣。
